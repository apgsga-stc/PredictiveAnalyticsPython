...
def map_stations(
    ...
    if nr_ranges is not None and canton is not None:
        for nr_range, station_name in nr_ranges.items():
            target_rows = (spr_data["Gebiet Code"] == canton) & (
                spr_data["Nummer"].isin(range(nr_range.low, nr_range.high + 1))
            )
            spr_data.loc[target_rows, "Station"] = station_name
...
map_stations(canton="GE", nr_ranges={NrRange(680802, 690726): "Genève Cornavin",})
map_stations(
    canton="VD",
    nr_ranges={
