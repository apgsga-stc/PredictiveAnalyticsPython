...
class Connection:
    ...
    def _connect(self, instance, user, passwd):
        ...
        try:
            conn = orcl.connect(user, passwd, self._connect_str(instance), encoding = 'UTF-8')
        except orcl.DatabaseError as d:
            err_msg = d.args[0].message
            raise orcl.DatabaseError(f'Connection to {user}/{passwd}@{instance} failed: {err_msg}')
        except:
            raise
        else:
            return conn
...
if __name__ == "__main__":
    ...
    with Connection('IT21_DEV_VK') as it21_dev:
        test = it21_dev.query("select 'It' HELLO, 'works!' WORLD from dual", squeeze=False)
    ...
    print(conn.query("select 1 nr, 'abc' txt from dual", squeeze=True, squeeze_axis='rows'))
