...
def flatten(obj):
    """Flatten complex object recursively to linear sequence. Supports nesting levels up to the interpreter's recursion depth limit."""
...
def _total_size(obj, handlers=OrderedDict()):
    ...
    def sizeof(obj):
        if id(obj) in seen:       # do not double count the same object
            return 0
        seen.add(id(obj))
        bytes = sys.getsizeof(obj, DFLT_SIZE)
        for typ, handler in all_handlers.items():
            if isinstance(obj, typ):
                bytes += sum(map(sizeof, handler(obj)))
                break
        return bytes
...
def obj_size(obj, format=True):
    """Return memory usage of a python object."""
    if isinstance(obj, pd.DataFrame):
        bytes = obj.memory_usage(deep=True).sum()
    elif isinstance(obj, pd.Series):
        bytes = obj.memory_usage(deep=True)
    else:
        bytes = _total_size(obj)
    if format:
...
if __name__ == "__main__":
    print(seq_join((f"{bytes} bytes are {format_size(bytes)}" for bytes in [1024, 123456, 123456789]), sep='\n'))
    ...
    testtab = pd.DataFrame.from_records(
        ...
        data    = [(d, week(d), iso_week(d), iso_week_rd(d, 2), iso_week_rd(d, 4)) for d in year_days],
        index   = 'date' 
    )
    print(testtab)
